...
public:
    ...
    int changed;
    int currentlinebelong;
    //构造函数
    Node(){};
    Node(int s, double dist,int change=0,int clb=0) :stationID(s), distance(dist),changed(change),currentlinebelong(clb)
    {};

    //">"运算重载，用于小顶堆
    bool operator > (const Node& n) const
    {
        return this->distance+this->changed>n.distance+n.changed;
    }
};
...
public:
    ...
    bool queryTransferMinTime(int s1, int s2,
                              ...
                              QList<Edge>&edgesList,int &distance,int &CT);
    //获取最少换乘的线路
    bool queryTransferMinTransfer(int s1, int s2,
                                  QList<int>&stationsList,
                                  QList<Edge>&edgesList,int &distance,int &CT);
    bool queryTransferMinStation(int s1, int s2,
                                 QList<int>&stationsList,
                                 QList<Edge>&edgesList,int &distance,int &CT);
