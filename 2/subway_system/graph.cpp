...
void SubwayGraph::makeGraph()
{
    ...
    for (auto &a : edges)
    {
        ...
        graph[a.first].push_back(Node(a.second, dist,0,stations[a.second].linesInfo.toList().first()));
        graph[a.second].push_back(Node(a.first, dist,0,stations[a.first].linesInfo.toList().first()));
    }
...
//获取最少时间的线路
bool SubwayGraph::queryTransferMinTime(int s1, int s2, QList<int>&stationsList, QList<Edge>&edgesList,int &distance,int &CT)
{
#define INF 999999
    stationsList.clear();
    edgesList.clear();
    ...
    std::vector<double> dist(stations.size(), INF);
     std::vector<int> changed(stations.size(),INF);
    dist[s1]=0;
    changed[s1]=0;
    std::priority_queue<Node, std::vector<Node>, std::greater<Node>> priQ;
    priQ.push(Node(s1, 0,0,stations[s1].linesInfo.toList().first()));
    while(!priQ.empty())
    {
        Node top=priQ.top();
        priQ.pop();
        ...
        for (int i=0; i<graph[top.stationID].size(); ++i)
        {
            ...
            if(top.distance+adjNode.distance<dist[adjNode.stationID])
            {
                ...
                QVector<int> befor=stations[(path[top.stationID]>=0 ? path[top.stationID]:top.stationID)].linesInfo.toList().toVector();
                QVector<int> next=stations[adjNode.stationID].linesInfo.toList().toVector();
                for(auto i=befor.begin();i!=befor.end();++i)
                {
                    for(auto j=next.begin();j!=next.end();++j)
                    {
                        if(*i==*j && stations[top.stationID].linesInfo.contains(*i))
                            nochanged=1;
                    }
                }
                changed[adjNode.stationID]=changed[top.stationID]+!nochanged;
                priQ.push(Node(adjNode.stationID, dist[adjNode.stationID],0,adjNode.currentlinebelong));
                //qDebug()<<adjNode.stationID<<dist[adjNode.stationID];
            }
        }
    }
...
//获取最少换乘的线路
bool SubwayGraph::queryTransferMinTransfer(int s1, int s2, QList<int>&stationsList, QList<Edge>&edgesList,int &distance,int &CT)
{
    ...
    std::priority_queue<Node, std::vector<Node>, std::greater<Node>> priQ;
    priQ.push(Node(s1, 0,0,stations[s1].linesInfo.toList().first()));
    qDebug()<<stations[s1].linesInfo.toList().first();
    while(!priQ.empty())
    {
        Node top=priQ.top();
        priQ.pop();
        ...
        {
            Node &adjNode=graph[top.stationID][i];
//            if(top.distance+adjNode.distance+1000.*(top.currentlinebelong!=adjNode.currentlinebelong)
//                    < dist[adjNode.stationID]+changed[adjNode.stationID])
            if(top.distance+adjNode.distance
                    < dist[adjNode.stationID])
            {
                path[adjNode.stationID]=top.stationID;
                dist[adjNode.stationID]=top.distance+adjNode.distance;
                int nochanged=0;
                ...
                changed[adjNode.stationID]=top.changed+times*(!nochanged);
                priQ.push(Node(adjNode.stationID, dist[adjNode.stationID],changed[adjNode.stationID],adjNode.currentlinebelong));
                //qDebug()<<adjNode.stationID<<dist[adjNode.stationID]<<changed[adjNode.stationID]<<adjNode.currentlinebelong;
            }
        }
    }
...
bool SubwayGraph::queryTransferMinStation(int s1, int s2, QList<int> &stationsList, QList<Edge> &edgesList,int &distance,int &CT)
{
    ...
    for (auto &a : edges)
    {
        ...
        graph[a.first].push_back(Node(a.second, 1,0,stations[a.second].linesInfo.toList().first()));
        graph[a.second].push_back(Node(a.first, 1,0,stations[a.first].linesInfo.toList().first()));
    }
    ...
    while(path[p]!=-1)
    {
        stationsList.push_front(p);
        edgesList.push_front(Edge(path[p],p));
        distance+=stations[p].distance(stations[path[p]]);
        p=path[p];
    }
    CT=changed[s2];
    stationsList.push_front(s1);
