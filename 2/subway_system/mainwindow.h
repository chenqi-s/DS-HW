...
private slots:
    ...
    void on_actionQueryInfo_triggered();
...
protected:
    ...
    Solution_show* solution;
    ChartView *compare;

    QLabel *statusLabel1, *statusLabel2;
    int way;
    QVector<int > M1,M2,M3;
    //连接信号和槽函数
    void myConnect();
    //初始状态栏
    void initStatusBar();
    ...
    //绘制网络图的边
    void drawEdges (const QList<Edge>& edgesList,int choose=0);
    //绘制网络图的站点节点
    void drawStations (const QList<int>& stationsList,int choose=0);
